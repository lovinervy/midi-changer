...
class MidiChanger:
    ...
    def create_note_segment(
        ...
        if len(note_segment) > duration_ms:
            sustain_segment = note_segment[
                duration_ms : duration_ms + sustain_ms
            ].fade_out(sustain_ms)
    ...
    def create_timing(self, track: list):
        ...
        for msg in track:
            ...
            time += msg.time
            current_time_ms = (
                tick2second(time, self.midi.ticks_per_beat, self.tempo) * 1000
            )  # Конвертирует в миллисекунды
    ...
    def create_track(self, channel: int):
        ...
        channel_play = sorted(
            self.channel_play[channel],
            key=lambda x: (x[2], "on" if x[0] == "on" else "off"),
        )

        initial_len = len(channel_play)
        ...
        while len(channel_play) > 1:
            ...
            for i, _data in enumerate(channel_play):
                if _data[0] == "off" and _data[1] == data[1]:
                    ...
                    if duration_ms == 0.0:
                        break
                    segment = self.create_note_segment(
                        "piano", data[1], duration_ms, int(duration_ms / 2)
                    )
                    audio = audio.overlay(segment, position=data[2])
                    break
            progress = (initial_len - len(channel_play)) / (initial_len - 1)
            bar_len = 30
